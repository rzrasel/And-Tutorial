...
public class LatinKeyboardBaseView extends View implements UIProxy {
    ...
    private boolean useEmojiFont;

    class C00391 extends SimpleOnGestureListener {
        C00391() {
        }
    ...
    class C00602 implements OnKeyboardActionListener {
        C00602() {
        }
    ...
    public LatinKeyboardBaseView(Context context, AttributeSet attrs) {
        this(context, attrs, C0053R.attr.keyboardViewStyle);
    }
    ...
    public LatinKeyboardBaseView(Context context, AttributeSet attrs, int defStyle) {
        ...
        this.emojiFont = null;
        TypedArray a = context.obtainStyledAttributes(attrs, C0053R.styleable.LatinKeyboardBaseView, defStyle, C0053R.style.LatinKeyboardBaseView);
        ...
        this.mHintTextSize = res.getDimension(C0053R.dimen.corner_hint_text_size);
        this.mHintToLeft = res.getInteger(C0053R.integer.hint_to_left);
        this.mHintToUp = res.getInteger(C0053R.integer.hint_to_up);
        this.mHintBottomPaddingPlus = 7;
        this.mPreviewPopup = new PopupWindow(context);
        if (previewLayout != 0) {
            this.mPreviewText = (TextView) inflate.inflate(previewLayout, null);
            this.mPreviewTextSizeLarge = (int) res.getDimension(C0053R.dimen.key_preview_text_size_large);
        ...
        this.mPreviewPopup.setAnimationStyle(C0053R.style.KeyPreviewAnimation);
        this.mDelayBeforePreview = res.getInteger(C0053R.integer.config_delay_before_preview);
        this.mDelayAfterPreview = res.getInteger(C0053R.integer.config_delay_after_preview);
        ...
        this.mMiniKeyboardPopup.setAnimationStyle(C0053R.style.MiniKeyboardAnimation);
        ...
        this.mDisambiguateSwipe = res.getBoolean(C0053R.bool.config_swipeDisambiguation);
        this.mMiniKeyboardSlideAllowance = res.getDimension(C0053R.dimen.mini_keyboard_slide_allowance);
        this.mGestureDetector = new GestureDetector(getContext(), new C00391(), null, true);
        ...
        this.mKeyRepeatInterval = res.getInteger(C0053R.integer.config_key_repeat_interval);
    }
    ...
    public void setKeyboard(Keyboard keyboard) {
        ...
        this.mKeyboardVerticalGap = (int) getResources().getDimension(C0053R.dimen.key_bottom_gap);
        Iterator it = this.mPointerTrackers.iterator();
        while (it.hasNext()) {
    ...
    private void onBufferDraw() {
        ...
        if (this.mKeyboard != null) {
            ...
            if (invalidKey != null && canvas.getClipBounds(clipRegion) && (invalidKey.f0x + kbdPaddingLeft) - 1 <= clipRegion.left && (invalidKey.f1y + kbdPaddingTop) - 1 <= clipRegion.top && ((invalidKey.f0x + invalidKey.width) + kbdPaddingLeft) + 1 >= clipRegion.right && ((invalidKey.f1y + invalidKey.height) + kbdPaddingTop) + 1 >= clipRegion.bottom) {
                drawSingleKey = true;
            }
            canvas.drawColor(0, Mode.CLEAR);
            Typeface banglaASCIIFont = Typefaces.get(getContext(), LatinIME.BANGLA_ASCII_FONT);
            this.useEmojiFont = true;
            if (this.mKeyboard.isEmoji && this.useEmojiFont && this.emojiFont == null) {
                try {
                    this.emojiFont = Typeface.createFromAsset(getContext().createPackageContext("ridmik.plugins.coloremoji", 0).getAssets(), "fonts/notocoloremoji.ttf");
                } catch (Exception e) {
                }
            }
            for (Key key : keys) {
                if (!drawSingleKey || invalidKey == key) {
                    ...
                    canvas.translate((float) (key.f0x + kbdPaddingLeft), (float) (key.f1y + kbdPaddingTop));
                    keyBackground.draw(canvas);
                    if (this.themeIndex == LatinIME.LOLIPOP_THEME_INDEX || this.themeIndex == LatinIME.LOLIPOP_THEME_INDEX + 1) {
                        if (key.codes != null && key.codes[0] == 32) {
                            Drawable dr;
                            if (this.themeIndex == LatinIME.LOLIPOP_THEME_INDEX) {
                                dr = getResources().getDrawable(C0053R.drawable.btn_keyboard_key_popup_selected_lxx_dark);
                            } else {
                                dr = getResources().getDrawable(C0053R.drawable.btn_keyboard_key_popup_selected_lxx_light);
                            }
                            dr.setBounds(0, 0, key.width, key.height);
                            dr.draw(canvas);
                        }
                        if (key.codes != null && (key.codes[0] == -1 || key.codes[0] == LatinIME.FIXED_BANGLA_NORMAL || key.codes[0] == LatinIME.FIXED_BANGLA_SHIFTED)) {
                            if (this.themeIndex == LatinIME.LOLIPOP_THEME_INDEX) {
                                if (getKeyboard().isShifted()) {
                                    key.icon = getResources().getDrawable(C0053R.drawable.sym_keyboard_shift_locked_lxx_dark);
                                } else {
                                    key.icon = getResources().getDrawable(C0053R.drawable.sym_keyboard_shift_lxx_dark);
                                }
                            } else if (getKeyboard().isShifted()) {
                                key.icon = getResources().getDrawable(C0053R.drawable.sym_keyboard_shift_locked_lxx_light);
                            } else {
                                key.icon = getResources().getDrawable(C0053R.drawable.sym_keyboard_shift_lxx_light);
                            }
                        }
                        if (key.codes != null && key.codes[0] == 10) {
                            key.icon = getResources().getDrawable(C0053R.drawable.enter);
                        }
                        if (key.codes != null && key.codes[0] == -5) {
                            if (this.themeIndex == LatinIME.LOLIPOP_THEME_INDEX) {
                                key.icon = getResources().getDrawable(C0053R.drawable.sym_keyboard_delete_lxx_dark);
                            } else {
                                key.icon = getResources().getDrawable(C0053R.drawable.sym_keyboard_delete_lxx_light);
                            }
                        }
                    }
                    ...
                    if (label != null) {
                        ...
                        if (key.codes != null && key.codes[0] > 9000 && this.useEmojiFont && this.emojiFont != null) {
                            paint.setTypeface(this.emojiFont);
                        } else if (useBanglaFont()) {
                            paint.setTypeface(banglaASCIIFont);
                        }
    ...
    private void showKey(int keyIndex, PointerTracker tracker) {
        ...
        if (key != null) {
            ...
            if (key.codes != null && key.codes[0] > 9000 && this.useEmojiFont && this.emojiFont != null) {
                ...
                this.mPreviewText.setTypeface(this.emojiFont);
                this.mPreviewText.setText(key.label);
            } else if (key.icon == null || shouldDrawLabelAndIcon(key)) {
                this.mPreviewText.setCompoundDrawables(null, null, null, null);
                ...
                if (key.label.length() <= 1 || key.codes.length >= 2) {
                    ...
                    if (useBanglaFont()) {
                        this.mPreviewText.setTypeface(banglaASCIIFont);
                    } else {
            ...
            int popupPreviewX = key.f0x - ((popupWidth - key.width) / 2);
            int popupPreviewY = (key.f1y - popupHeight) + this.mPreviewOffset;
            ...
            if (this.mWindowY + popupPreviewY < 0) {
                if (key.f0x + key.width <= getWidth() / 2) {
    ...
    public void invalidateKey(Key key) {
        if (key != null) {
            ...
            this.mDirtyRect.union(key.f0x + getPaddingLeft(), key.f1y + getPaddingTop(), (key.f0x + key.width) + getPaddingLeft(), (key.f1y + key.height) + getPaddingTop());
            onBufferDraw();
            invalidate(key.f0x + getPaddingLeft(), key.f1y + getPaddingTop(), (key.f0x + key.width) + getPaddingLeft(), (key.f1y + key.height) + getPaddingTop());
        }
    }
    ...
    private View inflateMiniKeyboardContainer(Key popupKey) {
        ...
        Keyboard keyboard;
        LatinKeyboardBaseView miniKeyboard = (LatinKeyboardBaseView) container.findViewById(C0053R.id.LatinKeyboardBaseView);
        miniKeyboard.setOnKeyboardActionListener(new C00602());
    ...
    protected boolean onLongPress(Key popupKey) {
        ...
        this.mMiniKeyboard = (LatinKeyboardBaseView) container.findViewById(C0053R.id.LatinKeyboardBaseView);
        ...
        int popupX = (popupKey.f0x + this.mWindowOffset[0]) + getPaddingLeft();
        ...
        int popupY = (((popupKey.f1y + this.mWindowOffset[1]) + getPaddingTop()) - container.getMeasuredHeight()) + container.getPaddingBottom();
        int x = popupX;
        ...
        this.mMiniKeyboardPopup.setHeight(this.mPreviewHeight);
        this.mMiniKeyboardPopup.showAtLocation(this, 0, x, y);
        long eventTime = SystemClock.uptimeMillis();
        this.mMiniKeyboardPopupTime = eventTime;
        MotionEvent downEvent = generateMiniKeyboardMotionEvent(0, (popupKey.width / 2) + popupKey.f0x, (popupKey.height / 2) + popupKey.f1y, eventTime);
    ...
    public boolean onTouchEvent(MotionEvent me) {
        ...
        if (this.mMiniKeyboard != null) {
            ...
            if (miniKeyboardPointerIndex >= 0 && miniKeyboardPointerIndex < pointerCount) {
                int miniKeyboardX = (int) me.getX(miniKeyboardPointerIndex);
                int y2 = (int) me.getY(miniKeyboardPointerIndex);
                int i = (this.themeIndex == LatinIME.LOLIPOP_THEME_INDEX || this.themeIndex == LatinIME.LOLIPOP_THEME_INDEX + 1) ? 50 : 0;
                MotionEvent translated = generateMiniKeyboardMotionEvent(action, miniKeyboardX, y2 - i, eventTime);
        ...
        if (this.mHasDistinctMultitouch) {
            ...
            for (int i2 = 0; i2 < pointerCount; i2++) {
                getPointerTracker(me.getPointerId(i2)).onMoveEvent((int) me.getX(i2), (int) me.getY(i2), eventTime);
            }
            return true;
        }

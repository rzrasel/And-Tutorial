...
public class PointerTracker {
    ...
    public PointerTracker(int id, UIHandler handler, KeyDetector keyDetector, UIProxy proxy, Resources res, int longPressDelay) {
        ...
        this.mDelayBeforeKeyRepeatStart = res.getInteger(C0053R.integer.config_delay_before_key_repeat_start);
        this.mLongPressKeyTimeout = longPressDelay;
        this.mMultiTapKeyTimeout = res.getInteger(C0053R.integer.config_multi_tap_key_timeout);
        resetMultiTap();
    }
    ...
    public void repeatKey(int keyIndex) {
        Key key = getKey(keyIndex);
        if (key != null) {
            detectAndSendKey(keyIndex, key.f0x, key.f1y, -1);
        }
    }
    ...
    private static int getSquareDistanceToKeyEdge(int x, int y, Key key) {
        int left = key.f0x;
        int right = key.f0x + key.width;
        int top = key.f1y;
        int bottom = key.f1y + key.height;

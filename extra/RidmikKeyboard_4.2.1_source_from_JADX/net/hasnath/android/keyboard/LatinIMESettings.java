...
public class LatinIMESettings extends PreferenceActivity implements OnSharedPreferenceChangeListener, OnDismissListener {
    ...
    class C00361 implements OnPreferenceClickListener {
        C00361() {
        }
    ...
    class C00372 implements OnPreferenceClickListener {
        C00372() {
        }

        public boolean onPreferenceClick(Preference preference) {
            LatinIMESettings.this.startActivity(new Intent("android.intent.action.VIEW", Uri.parse("market://details?id=ridmik.plugins.coloremoji")));
            return true;
        }
    }

    class C00383 implements OnClickListener {
        C00383() {
        }
    ...
    protected void onCreate(Bundle icicle) {
        ...
        addPreferencesFromResource(C0053R.xml.prefs);
        ...
        this.mVoiceModeOff = getString(C0053R.string.voice_mode_off);
        this.mVoiceOn = !prefs.getString(VOICE_SETTINGS_KEY, this.mVoiceModeOff).equals(this.mVoiceModeOff);
        this.mLongPressDelay = prefs.getInt(PREF_LONG_PRESS_DELAY, getResources().getInteger(C0053R.integer.config_long_press_key_timeout));
        this.mLogger = VoiceInputLogger.getLogger(this);
        findPreference("ridmik_help").setOnPreferenceClickListener(new C00361());
        findPreference("colorEmoji").setOnPreferenceClickListener(new C00372());
    }
    ...
    private void updateSettingsKeySummary() {
        this.mSettingsKeyPreference.setSummary(getResources().getStringArray(C0053R.array.settings_key_modes)[this.mSettingsKeyPreference.findIndexOfValue(this.mSettingsKeyPreference.getValue())]);
    }

    private void updateBanglaFixedKeySummary() {
        this.mBanglaFixedKeyPreference.setSummary(getResources().getStringArray(C0053R.array.bangla_fixed_modes)[this.mBanglaFixedKeyPreference.findIndexOfValue(this.mBanglaFixedKeyPreference.getValue())]);
    }

    private void updateKeyboardLayoutKeySummary() {
        this.mKeyboardTheme.setSummary(getResources().getStringArray(C0053R.array.keyboard_layout_modes)[this.mKeyboardTheme.findIndexOfValue(this.mKeyboardTheme.getValue())]);
    }
    ...
    private void updateLongPressDelaySummary() {
        this.mLongPressDelayPreference.setSummary(String.format(getResources().getString(C0053R.string.long_press_delay_summary), new Object[]{Integer.valueOf(this.mLongPressDelay)}));
    }
    ...
    private void updateVoiceModeSummary() {
        this.mVoicePreference.setSummary(getResources().getStringArray(C0053R.array.voice_input_modes_summary)[this.mVoicePreference.findIndexOfValue(this.mVoicePreference.getValue())]);
    }

    protected Dialog onCreateDialog(int id) {
        switch (id) {
            case 0:
                OnClickListener listener = new C00383();
                Builder builder = new Builder(this).setTitle(C0053R.string.voice_warning_title).setPositiveButton(17039370, listener).setNegativeButton(17039360, listener);

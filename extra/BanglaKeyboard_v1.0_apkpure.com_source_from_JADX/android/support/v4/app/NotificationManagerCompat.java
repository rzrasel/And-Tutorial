    ...
    class SideChannelManager implements ServiceConnection, Callback {
        ...
        private void processListenerQueue(ListenerRecord listenerRecord) {
            if (Log.isLoggable(NotificationManagerCompat.TAG, 3)) {
                "Processing component " + listenerRecord.componentName + ", " + listenerRecord.taskQueue.size() + " queued tasks";
            }
            if (!listenerRecord.taskQueue.isEmpty()) {
                ...
                while (true) {
                    ...
                    try {
                        ...
                        if (Log.isLoggable(NotificationManagerCompat.TAG, 3)) {
                            "Remote service has died: " + listenerRecord.componentName;
                        }
                    } catch (Throwable e2) {
            ...
            if (!enabledListenerPackages.equals(this.mCachedEnabledPackages)) {
                ...
                while (it.hasNext()) {
                    ...
                    if (!hashSet.contains(entry.getKey())) {
                        if (Log.isLoggable(NotificationManagerCompat.TAG, 3)) {
                            "Removing listener record for " + entry.getKey();
                        }
